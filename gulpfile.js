...
gulp.task('prepareForDeploy', () => {
  ...
  return run('python salesforce/visualforce_transform.py --assets sftestassets --controller NewHomePageCtrl --extensions NewHomePageCtrl --pagename sftestpage --builddir src-dev-deploy').exec('', function(e) {
    if (e && e.status) {
      console.log("Pre-prep script failed. NOT DEPLOYING", e);
      if (exitOnFailure) {
        process.exit(1);
      }
    } else {
